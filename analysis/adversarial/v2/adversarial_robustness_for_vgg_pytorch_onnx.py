...
def generate_correctly_classified_data(valloader, y_test, pytorch_model, ort_session, total_number=1000):
    X_correctly_classifier = np.zeros(shape=(total_number, 3, 224, 224))
    y_correctly_classifier = np.zeros(shape=(total_number, ))
    counter = 0
    for i, (data, labels) in enumerate(valloader):
        labels = labels.numpy()
        for j in range(len(data)):
            sample = data[j].reshape(1, 3, 224, 224)
            x = torch.tensor(sample, dtype=torch.float)
            #x = x.cuda()
            torch_out = pytorch_model(x)
            ort_inputs = {ort_session.get_inputs()[0].name: sample.numpy()}
            onnx_output = ort_session.run(None, ort_inputs)[0]
            ground_truth = labels[j]
            value, pred = torch.max(torch_out, 1)
            pred = pred.data.cpu()
            pytorch_prediction = pred.numpy()[0]
            onnx_prediction = np.argmax(onnx_output, axis=1)[0]

            if ground_truth == pytorch_prediction and ground_truth == onnx_prediction:
...
def generate_original_data(pytorch_model_path, onnx_model_path, original_file_path, model_names, valloader, y_test):
    for model_name in model_names:
        pytorch_path = pytorch_model_path + model_name + ".pth"
        onnx_path = onnx_model_path + model_name + ".onnx"
...
def generate_results(pytorch_model_path, onnx_model_path, adversarial_file_path, original_file_path, model_names):
    ...
    for i, model_name in enumerate(model_names):
        ...
        ort_inputs_fgsm = {ort_session.get_inputs()[0].name: X_adv_fgsm}
        ort_outs_fgsm = ort_session.run(None, ort_inputs_fgsm)
        onnx_predictions_fgsm = np.argmax(ort_outs_fgsm[0], axis=1)
        ort_inputs_boundary = {ort_session.get_inputs()[0].name: X_adv_boundary}
        ort_outs_boundary = ort_session.run(None, ort_inputs_boundary)
        onnx_predictions_boundary = np.argmax(ort_outs_boundary[0], axis=1)
        ...
        misclassification_after_cov_fgsm = 100 * (np.sum(onnx_predictions_fgsm != ground_truth_fgsm) / len(ground_truth_fgsm))
        misclassification_after_cov_boundary = 100 * (np.sum(onnx_predictions_boundary != ground_truth_boundary) / len(ground_truth_boundary))

        conversion_divergence_fgsm = np.sum(onnx_predictions_fgsm != pytorch_predictions_fgsm)
        conversion_divergence_boundary = np.sum(onnx_predictions_boundary != pytorch_predictions_boundary)

        abs_err_fgsm = np.mean(np.absolute(pytorch_output_fgsm - ort_outs_fgsm[0]))
        abs_err_boundary = np.mean(np.absolute(pytorch_output_boundary - ort_outs_boundary[0]))
...
def main():
    ...
    pytorch_model_path = "/Volumes/Cisco/Fall2021/onnx-exchange/Training/pytorch/vgg/"
    onnx_model_path = "/Volumes/Cisco/Fall2021/onnx-exchange/conversion/onnx/pytorch/vgg/"
    original_file_path = "./data/original/pytorch/vgg16/"
    adversarial_file_path = "./data/adversarial/pytorch/vgg16/"
    if not os.path.exists(original_file_path):
        Path(original_file_path).mkdir(parents=True, exist_ok=True)
    if not os.path.exists(adversarial_file_path):
        Path(adversarial_file_path).mkdir(parents=True, exist_ok=True)

    model_names = ["torch_exp_vgg_2021-11-04_9",
                   "torch_exp_vgg_2021-11-04_10"]

    model_names2 = ["torch_exp_vgg_2021-11-03_1", "torch_exp_vgg_2021-11-03_2", "torch_exp_vgg_2021-11-03_3",
    ...
    #                  type_attack="FGSM")
    generate_adv_data(pytorch_model_path, adversarial_file_path, original_file_path, model_names2, valloader, x_test,
                      type_attack="Boundary")
    generate_results(pytorch_model_path, onnx_model_path, adversarial_file_path, original_file_path, model_names2)
