...
def generate_correctly_classified_data(valloader, y_test, pytorch_model, coreml_model, total_number=500):
    ...
    for i, (data, labels) in enumerate(valloader):
        ...
        for j in range(len(data)):
            ...
            #print(coreml_output)
            coreml_prediction = np.argmax(coreml_output['var_313'][0])
...
def generate_adv_data(pytorch_model_path, adversarial_file_path, original_file_path, model_names, valloader, x_test,
    ...
    for model_name in model_names:
        ...
        # prepare attack
        model_ft, input_size = initialize_model("resnet", num_classes=10, feature_extract=False, use_pretrained=True)
...
def generate_results(pytorch_model_path, coreml_model_path, adversarial_file_path, original_file_path, model_names):
    ...
    for i, model_name in enumerate(model_names):
        ...
        coreml_predictions_fgsm = np.argmax(coreml_output_fgsm['var_313'], axis=1)
        coreml_output_boundary = coreml_model.predict({name_1: X_adv_boundary})
        coreml_predictions_boundary = np.argmax(coreml_output_boundary['var_313'], axis=1)
        ...
        abs_err_fgsm = np.mean(np.absolute(pytorch_output_fgsm - coreml_output_fgsm['var_313']))
        abs_err_boundary = np.mean(np.absolute(pytorch_output_boundary - coreml_output_boundary['var_313']))
    ...
    file_name = "Resnet_pytorch_onnx.xlsx"
    ...
    results_df.to_excel(writer, sheet_name='Restnet_pytorch_onnx')
...
def main():
    ...
    #pytorch_model_path = "../../Training/pytorch/resnet/"
    #onnx_model_path = "../../conversion/onnx/pytorch/resnet18/"
    #original_file_path = "../data/original/pytorch/resnet18/"
    #adversarial_file_path = "../data/adversarial/pytorch/resnet18/"

    pytorch_model_path = "/Volumes/Cisco/Fall2021/onnx-exchange/Training/pytorch/resnet18/"
    coreml_model_path = "/Volumes/Cisco/Fall2021/onnx-exchange/conversion/coremltools/pytorch/resnet18/"
    original_file_path = "/Volumes/Cisco/Summer2022/onnx-exchange/adversarial/original/pytorch/resnet18/"
    adversarial_file_path = "/Volumes/Cisco/Summer2022/onnx-exchange/adversarial/adversarial/pytorch/resnet18/"

    model_names = ["torch_exp_resnet_2021-11-04_1", "torch_exp_resnet_2021-11-04_2", "torch_exp_resnet_2021-11-04_3",
                   "torch_exp_resnet_2021-11-06_4", "torch_exp_resnet_2021-11-06_5", "torch_exp_resnet_2021-11-06_6",
                   "torch_exp_resnet_2021-11-06_7", "torch_exp_resnet_2021-11-06_8", "torch_exp_resnet_2021-11-06_9",
                   "torch_exp_resnet_2021-11-06_10"]
